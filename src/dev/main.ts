...
const travelplan_source_url = '/travelplan4.json';
...
const segmentTypes = new SegmentTypes();
...
async function documentDomLoaded() {
  ...
  accoTable.renderElementDescription = function (element) {
    return `${element.nights} ${element.nights == 1 ? 'nacht' : 'nachten'} - ${
      element.title
